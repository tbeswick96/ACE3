...
if (_isActive || { CBA_missionTime >= _timeWhenActive }) then {
    ...
    if ((_lastTargetPollTime + ACTIVE_RADAR_POLL_FREQUENCY) - CBA_missionTime < 0) then {
        ...
        // Only re-survey for a new return once the current target falls out of the gate.
        if (
            !isNull _target
            && {alive _target}
        ...
        private _a2 = 180 - (_seekerAngle + _a1);
        private _seekerBaseRadiusAtGround = ACTIVE_RADAR_MINIMUM_SCAN_AREA max (_distanceToExpectedTarget / sin(_a2) * sin(_seekerAngle));
