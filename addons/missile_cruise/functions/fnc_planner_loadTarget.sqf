...
/*
 * Author: UKSF
 * Loads the active target hashmap entry into the dialog UI fields (PP mode),
 * or loads TOO settings (TOO mode).
...
if (GVAR(plannerMode) isEqualTo "too") then {
    // TOO mode: load only editable fields from TOO settings
    private _impactAngle = GVAR(tooSettings) get "impactAngle";
    private _attackHeading = GVAR(tooSettings) get "attackHeading";
    private _cruiseAltitude = GVAR(tooSettings) get "cruiseAltitude";

    // Coordinate fields are driven by PFH — don't touch them here
    ...
    // Target label
    (_display displayCtrl CRUISE_PLANNER_IDC_TGT_LABEL) ctrlSetText "TOO";

    // Clear waypoint list
    lbClear (_display displayCtrl CRUISE_PLANNER_IDC_LIST);
    (_display displayCtrl CRUISE_PLANNER_IDC_INFO) ctrlSetText "TOO Mode";

    TRACE_1("planner_loadTarget TOO",_attackHeading);
} else {
    // PP mode: load from active target entry
    ...
    TRACE_2("planner_loadTarget PP",GVAR(activeTarget),_position);
};
