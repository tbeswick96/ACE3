 ...
 * on the planner map control. In TOO mode, draws live TGP target
 * and line from aircraft. In PP mode, reads target from dialog input fields.
 *
 * Arguments:
 * None
 *
...
// Draw vehicle position - green (both modes)
private _vehiclePos = getPos _vehicle;
private _vehicle2D = [_vehiclePos select 0, _vehiclePos select 1];
_map drawIcon [WP_ICON, [0, 1, 0, 1], _vehicle2D, 16, 16, 0, "AC", 1, 0.04, "TahomaB", "right"];

if (GVAR(plannerMode) isEqualTo "too") then {
    // TOO mode: draw live TGP target and line from aircraft
    ...
    private _target2D = [_position select 0, _position select 1];

    // Target icon - red
    _map drawIcon [WP_ICON, [1, 0, 0, 1], _target2D, 24, 24, 45, "TGT", 1, 0.04, "TahomaB", "right"];

    // Line from aircraft to target
    _map drawLine [_vehicle2D, _target2D, [0, 1, 0, 0.6]];

    // Attack direction line + approach WP if heading is set
    ...
    if (_headingStr isNotEqualTo "") then {
        ...
        private _approachPos2D = _target2D vectorAdd _reverseDir;
    ...
    if (_wpCount > 0) then {
        ...
        if (count _lastWP >= 3) then {
            _map drawLine [_vehicle2D, [_lastWP#0, _lastWP#1], [0, 1, 0, 0.3]];
        };
    } else {
        if (_hasTarget) then {
            if (_approachPos2D isNotEqualTo []) then {
                _map drawLine [_vehicle2D, _approachPos2D, [0, 1, 0, 0.3]];
            } else {
                _map drawLine [_vehicle2D, _target2D, [0, 1, 0, 0.3]];
            };
        };
    };
};
